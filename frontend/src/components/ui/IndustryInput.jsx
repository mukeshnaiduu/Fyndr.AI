...
export default function IndustryInput({ label = 'Industry', name = 'industry', value, onChange, onSelect, clearOnSelect = false, placeholder = 'e.g., Technology, Finance', description, error, className }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                ...
                const results = await fetchIndustries(q);
                if (!cancelled) {
                    const next = results.map(r => r.name).slice(0, 8);
                    setSuggestions(next);
